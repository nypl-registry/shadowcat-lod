...
describe('API Holdings', function () {
	...
	it('checkOclcHoldingsTwoPagesForNYPL - it should return an object with both pages results', function (done) {
		...
		api.checkOclcHoldingsTwoPagesForNYPL('499832410','10016',function(results){
	...
	it('checkOclcHoldingsTwoPagesForNYPL - it should return an object with both pages results false', function (done) {
	


		api.checkOclcHoldingsTwoPagesForNYPL('681467808','10016',function(results){

			results.one.should.equal(-1)
			results.two.should.equal(-1)
			results.all.should.equal(false)
