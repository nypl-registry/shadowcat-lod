...
exports.returnNextApiWorldcatWork = function (cb) {
  MongoClient.connect(mongoConnectURL, function (err, db) {
    if (err) console.log(err)
...
exports.updateWorldcatData = function (dataStore, updateStartTime, cb) {
  MongoClient.connect(mongoConnectURL, function (err, db) {
    var collectionBib = (exports.testOverride) ? db.collection('test') : db.collection('bib')
    var collectionWorldcat = (exports.testOverride) ? db.collection('test') : db.collection('apiWorldcat')
      ...
      if (data) {
        if (data.complete && data.update && data.completeTime < updateStartTime) {
          collectionBib.update({ _id: data.update.id }, { $set: data.update }, function (err, result) {
            ...
            collectionWorldcat.remove({_id: data.update.id}, function (err, results) {
              if (err) console.log(err)
